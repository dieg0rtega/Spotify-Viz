...
const handleSubmit = async (event) => {
    ...
    const inputForm = document.getElementsByID('artist-input')
    const artistRequested = inputForm.values
    
    const artsits = await searchArtists(artistRequested)
    displayArtistCards(artists)
}

async function searchArtists(artist) {//search for an artists using Spotify's API
    const requestURL = 'https://api.spotify.com/v1/search?q=${artist}&type=artist&limit=5'
    const details = {
        method: "GET",
        headers: {
